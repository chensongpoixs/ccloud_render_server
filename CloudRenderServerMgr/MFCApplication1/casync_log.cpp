...
#error unexpected c complier (msc/gcc), Need to implement this method for demangle
	...
	{
		if (level < ELogLevel_None || level > ELogLevel_Num  || m_level_log < level)
		{
			return;
		}
		if (m_stoped)
	...
	//static const std::string  log_name_time = std::to_string(::time(NULL)) + "_test.log" ;
	//static FILE  * out_log_ptr = ::fopen(log_name_time.c_str(), "wb+");
	void casync_log::append_var(ELogLevelType level, const char * format, va_list ap)
	{
		//if (out_log_ptr)
		//{
		//	fprintf(out_log_ptr, "[%s][%d][m_level_log = %u][level = %u]\n", __FUNCTION__, __FILE__, m_level_log, level);
		//	fflush(out_log_ptr);
		//}
		...
		if (level < ELogLevel_None || level > ELogLevel_Num  || m_level_log < level)
		{
			//if (out_log_ptr)
			//{
			//	fprintf(out_log_ptr, "[%s][%d][m_level_log = %u][level = %u][warr]\n", __FUNCTION__, __FILE__, m_level_log, level);
			//	fflush(out_log_ptr);
			//}
			return;
		}
	...
	void casync_log::_handler_log_item(const clog_item * log_item_ptr)
	{
		...
		if (!(log_item_ptr->level == ELogLevel_Info || log_item_ptr->level == ELogLevel_System  ))
		{
